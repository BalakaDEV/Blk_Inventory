$(document).ready(function(){
	...
	document.onkeyup = function(data){
		if (data.which == 27){
			$.post("http://blk_inventory/invClose");
		}
	};
});
...
const updateDrag = () => {
	...
	$('.usar').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://blk_inventory/usarItem", JSON.stringify({
				item: itemData.key,
				type: itemData.type,
	...
	$('.dropar').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://blk_inventory/droparItem", JSON.stringify({
				item: itemData.key,
				type: itemData.type,
				amount: Number($("#amount").val())
			}))
	...
	$('.enviar').droppable({
		...
		drop: function(event,ui){
			...
			$.post("http://blk_inventory/enviarItem", JSON.stringify({
				item: itemData.key,
				type: itemData.type,
				amount: Number($("#amount").val())
			}))
...
const updateMochila = () => {
	document.getElementById("amount").value = "";
	$.post("http://blk_inventory/requestMochila",JSON.stringify({}),(data) => {
		...
		$('#inventario-items').html(`
			${nameList.map((item) => (`
				<div class="item" data-item-key="${item.key}" data-item-type="${item.type}" data-name-key="${item.name}">
					<div id="thumb"><img src='images/${item.index}.png'></div>	
		...
		$('.usar').html(`
			<img src="images/usar.png">
		`);
		$('.enviar').html(`
			<img src="images/enviar.png">
		`);
		$('.dropar').html(`
			<img src="images/dropar.png">
		`);
